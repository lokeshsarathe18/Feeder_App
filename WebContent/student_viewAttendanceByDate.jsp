...
<body>
	...
	<div style="margin-left: 30%">
		<span style="color: red; font-weight: bold; font-size: 20px;">Attendance
			Between Dates</span>
		<form method="post" action="student_viewAttendanceByDate.jsp">
			<%
				if (request.getMethod().equalsIgnoreCase("post")) {
			%>
			<input value="<%=request.getParameter("fdate")%>" required="required"
				placeholder="From Date" autocomplete="off" type="text" name="fdate" id="fdate" /> <input
				value="<%=request.getParameter("tdate")%>" required="required"
				placeholder="To Date" type="text" autocomplete="off" name="tdate" id="tdate" />
			<%
				} else {
			%>
			<input required="required" placeholder="From Date" type="text"
				name="fdate" autocomplete="off" id="fdate" /> <input required="required"
				placeholder="To Date" autocomplete="off" type="text" name="tdate" id="tdate" />
			<%
				}
			%>
	...
	<%
		if (request.getMethod().equalsIgnoreCase("post")) {
			...
			ArrayList<AttendanceDTO> al = new AttendanceDAOImp().getAttendanceStudentByDate(fdate, tdate, sdto.getEno());
			if (al != null) {
	%>
